...
export default function StudentRegistration({
  ...
  const handleSubmit = async () => {
    ...
    if (savedProgress && savedProgress.questions_json && savedProgress.config_json) {
      ...
      try {
        const savedQuestions = typeof savedProgress.questions_json === 'string' 
          ? JSON.parse(savedProgress.questions_json) 
          : savedProgress.questions_json;
        const savedConfig = typeof savedProgress.config_json === 'string'
          ? JSON.parse(savedProgress.config_json)
          : savedProgress.config_json;

        if (Array.isArray(savedQuestions) && savedQuestions.length > 0) {
          selected = savedQuestions;
          examConfigData.config = savedConfig;
          examConfigData.time = savedProgress.remaining_time_seconds || (savedConfig.tiempo_limite_minutos * 60);
          console.log("✅ Preguntas y configuración recuperadas exitosamente del progreso guardado");
        } else {
          throw new Error("Preguntas guardadas inválidas");
        }
      } catch (error) {
        console.log("Error recuperando progreso guardado, creando nuevas preguntas:", error);
        selected = [];
        areas.forEach((a) => {
          const numQuestions = selectedConfig[`preguntas_${a}`];
          if (numQuestions > 0 && gq[a]) {
            const pool = [...gq[a]];
            ...
            <button
              onClick={() => {
                const examConfigData = {
                  ...
                  time: savedProgress.remaining_time_seconds || (savedConfig.tiempo_limite_minutos * 60),
                  config: savedConfig
                };
                setExamConfig(examConfigData);
    ...
    <div className="bg-white/30 backdrop-blur-sm rounded-lg shadow-lg p-8">
      ...
      <div className="space-y-5">
        ...
        >
          ...
          {Object.keys(questionBank).sort((a, b) => {
