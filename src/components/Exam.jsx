...
export default function Exam({ student, config, onDone }) {
  ...
  useEffect(() => {
    // La limpieza del progreso se realiza después de confirmar que el examen
    // se guardó correctamente. Esto evita borrar el progreso si el guardado falla.
    // Este efecto se mantiene por compatibilidad pero no realiza ninguna acción.
  ...
  const finish = async () => {
    try {
      ...
      try {
        console.log("📤 Guardando examen para:", student.documento);
        console.log("📊 Resultados:", { score: resultados.score, tiempoUsado });
        
        const response = await examApi.guardarExamen(student.documento, resultados, tiempoUsado);
        
        console.log("✅ Respuesta del servidor:", response);
        
        if (response && response.examenId) {
          setExamProgressId(response.examenId);
          notify.push("success", "¡Examen guardado exitosamente!");
          // Nota: El progreso ya se elimina en el servidor dentro de la transacción de guardado
        } else {
          throw new Error("El servidor no devolvió un ID de examen válido");
        }
      } catch (error) {
        console.error("❌ Error crítico guardando en BD:", error);
        notify.push(
          "error",
          `Error al guardar: ${error.message}. Por favor, contacta al administrador.`
        );
        // No marcamos como finalizado si hubo un error crítico para permitir reintento si fuera posible
        // Pero en este caso, como ya calculamos resultados, mostramos el error claramente.
      }
